...
export default function TicketCard({ ticket, onClick }: TicketCardProps) {
  ...
  return (
    <div className="group cursor-pointer h-full transform-gpu transition-all duration-300 hover:-translate-y-1">
      {/* Ticket Container */}
      <div className="relative bg-white rounded-xl overflow-hidden h-full flex flex-col shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100">
        ...
        <div className="absolute left-0 top-0 bottom-0 w-[60px] bg-gray-50 border-r border-dashed border-gray-200 hidden sm:flex flex-col items-center justify-center z-10">
          <div className="bg-gradient-to-r from-blue-400 to-blue-600 text-white text-center py-1 px-2 text-xs font-bold w-full">
            {eventDate ? eventDate.toLocaleDateString("en-US", { month: "short" }).toUpperCase() : "TBA"}
          </div>
          <div className="text-center py-2 font-bold text-2xl text-gray-800">
          ...
          <div className="text-xs font-mono text-gray-500 rotate-90 tracking-widest mt-4">
            #{ticket.ticketNumber || ticket._id.substring(0, 8).toUpperCase()}
          </div>
        </div>
        ...
        <div className="sm:ml-[60px]">
          ...
          <div className="relative h-48 overflow-hidden">
            <Image
              src={ticket.event?.image || "/vibrant-tech-event.png"}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4">
              ...
              <p className="text-xs text-white/80 mt-1">
                Organized by {ticket.event?.organizerName || "Event Organizer"}
              </p>
            </div>
          </div>
            ...
            <div className="space-y-2 text-sm">
              ...
              {ticket.event?.location && (
                <div className="flex items-center">
                  ...
                  <span className="line-clamp-1 text-gray-700">{ticket.event?.location}</span>
                </div>
              )}
            </div>
